...
Kubernetes 학습 문서는 주제별로 나눠 관리합니다.
...
| `security/` | SecurityContext, 이미지 보안, Secret, TLS, RBAC Audit Log, Falco |
| `deep-dive/` | etcd, scheduler, CNI, admission, operator, autoscaling, upgrade 등 심화 주제 |
| `agents/` | AI 전문 에이전트 프롬프트 |
| `rules/` | 문서 작성과 Kubernetes 운영 규칙 |
| `templates/` | 재사용 문서 템플릿 |

실행 가능한 실습 코드는 `../ops/README.md`를 참고합니다.
